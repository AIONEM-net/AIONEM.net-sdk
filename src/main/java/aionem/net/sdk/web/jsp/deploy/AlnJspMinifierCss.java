...
@Log4j2
public class AlnJspMinifierCss {


    public static AlnDaoRes minifySave(String pathInOut) {
        return minifySave(pathInOut, pathInOut);
    }
    public static AlnDaoRes minifySave(String pathIn, String pathOut) {

        final AlnDaoRes resMinify = new AlnDaoRes();

        final File fileIn = new File(pathIn);
        final String css = minify(AlnUtilsText.toString(fileIn));

        final File fileOut = new File(pathOut);
        final boolean isSaved = AlnJspUtils.writeFile(fileOut, css);
    ...
    public static String minifyFolder(final AlnJsp alnJsp, final File fileFolder, final boolean isSave) {
        ...
        final Matcher matcher = pattern.matcher(AlnUtilsText.toString(fileCssJsp));
        ...
        for(int i = 0; i < listFileCss.size(); i++) {
            ...
            final File file = new File(alnJsp.getRealPathRoot(listFileCss.get(i)));
    ...
    public static String minify(String css) {
        ...
        try {
            ...
            List<AlnJspMinifierCssUtils.Selector> selectors = new ArrayList<>();
            n = 0;
            j = 0;
            ...
            for(AlnJspMinifierCssUtils.Selector selector : selectors) {
    ...
    private static class AlnJspMinifierCssUtils {
